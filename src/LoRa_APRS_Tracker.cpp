...
/* https://github.com/Slayingripper/Cubecell_LoRa_APRS_Tracker/tree/master */
...
void setup() {
  ...
  VextON(); // activate VEXT power
  setup_display();
  show_display("DJ1AN", "CubeCell", "LoRa APRS Tracker", 500);

  if (!bmp.begin()) {
    Serial.println(F("Could not find a valid BMP sensor, check wiring!"));
  } else {
    BMPFOUND = true;
  }
  ...
  if (LORA_LED) pinMode(LEDPIN, OUTPUT);
...
void loop() {
  ...
  if (send_update && gps_loc_update && gps_loc_valid) {
    ...
    if (LORA_LED) digitalWrite(LEDPIN, 1);
    is_txing = true;
...
void activateProfile(int profileNr) {
  DBEACON_LATITUDE = "";    //sticky variables!! ;)
  DBEACON_LONGITUDE = "";
  STATIC_BEACON = false;
  WEATHER_DATA = false;
